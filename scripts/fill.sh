...
curl -X POST "http://localhost:3500/api/equipos" -H "Content-Type: application/json" -d '{"nombre":"Wolfram City"}'
curl -X POST "http://localhost:3500/api/equipos" -H "Content-Type: application/json" -d '{"nombre":"USAD Champu"}'
